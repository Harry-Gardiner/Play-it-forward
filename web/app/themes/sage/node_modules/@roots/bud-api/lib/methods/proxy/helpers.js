...
export const assignPort = (bud, port) => {
    bud.hooks.on(`dev.middleware.proxy.options.target`, (url = fallbackUrl) => {
        url.port = `${port}`;
        return url;
    });
};
...
export const assignUrl = (bud, maybeURL) => {
    bud.hooks.on(`dev.middleware.proxy.options.target`, maybeURL instanceof URL ? maybeURL : new URL(maybeURL));
};
/**
