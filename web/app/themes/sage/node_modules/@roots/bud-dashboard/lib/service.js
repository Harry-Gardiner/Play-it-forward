        ...
        try {
            await Render(React.createElement(Ink.Box, { flexDirection: "column", marginTop: 1 },
                ...
                React.createElement(App, { compilations: compilations.map(compilation => ({
                    ...
                    })), displayAssets: true, displayEntrypoints: true, displayServerInfo: false, context: this.app.context, mode: this.app.mode, devUrl: this.app.hooks.filter(`dev.url`, new URL(`http://0.0.0.0:3000`)), proxyUrl: this.app.hooks.filter(`dev.middleware.proxy.options.target`), watchFiles: this.app.server?.watcher?.files })));
        }
        catch (error) { }
    }
    /**
